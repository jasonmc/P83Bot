...
let parseAndReply text sender isBot =
  ...
  | Prefix("@point83 ") rest ->
      ...
      | IgnoreCase("mikkeller nyc menu") true -> Mikkeller.getMenu () |> asyncToAsyncSeq |> AsyncSeq.concatSeq |> AsyncSeq.map Text
  ...
  | Contains(" Forest Park ") true when not isBot -> sprintf "NYC rule 7" |> Text |> asyncSeq.Yield
